...
export default function AvgeekConnect({ isOpen, onClose }) {
  ...
  const googleSdkInitializedRef = useRef(false); // Track whether Google Client GSI has been initialized
  ...
  // Handle Google Client Sign-In Credential Response
  const handleGoogleCredentialResponse = async (response) => {
    if (supabase) {
      try {
        ...
        setStatusMessage({ type: 'success', text: 'Successfully logged in with Google!' });
      } catch (err) {
        console.error('[aVgeek Connect] Google Token Auth Error:', err.message);
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.google && supabase) {
        if (!googleSdkInitializedRef.current) {
          window.google.accounts.id.initialize({
            client_id: '303855599601-4v0se7q81338m99cr9hc87snqk879uph.apps.googleusercontent.com',
            callback: handleGoogleCredentialResponse,
          });
          googleSdkInitializedRef.current = true;
        }

        const container = document.getElementById('google-client-signin-container');
        if (container) {
            ...
            <div className="flex-1 flex items-center justify-center py-4 sm:py-12 px-2 sm:px-4">
              <div className="bg-zinc-950/95 border border-white/10 p-5 sm:p-10 rounded-2xl shadow-2xl max-w-sm sm:max-w-md w-full text-center flex flex-col gap-4 sm:gap-6 select-none animate-slideUp mx-auto">
                ...
                <div className="flex flex-col gap-2.5 sm:gap-3 mt-1 sm:mt-2">
                  {/* Google Client SDK Sign-In Button Container */}
                  <div className="w-full flex justify-center items-center min-h-[44px]">
                    <div id="google-client-signin-container" className="w-full max-w-[280px]"></div>
                  </div>
